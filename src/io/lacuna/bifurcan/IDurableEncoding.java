 ...
 * <p>
 * For collections, we need only describe the encoding of the individual values.  For primitives, we must define
 * several things:
 * <ul>
 * <li>{@link Primitive#blockSize()}, which is the number of values which can be typically encoded in ~2kb</li>
 * <li>{@link Primitive#encode(IList, DurableOutput)}, which takes a list of values and writes them as a single block
 * to the output</li>
 * <li>{@link Primitive#decode(DurableInput, IDurableCollection.Root)} which takes an input corresponding to an
 * encoded block, and returns a special iterator that can skip over ignored values</li>
 * </ul>
 * <p>
 * Optionally, encodings may provide custom hash, equality, and comparison semantics.
 * <p>
 * One should not expect a 1:1 correspondence between encodings and in-memory types.  Where an in-memory type only
 * relates
...
public interface IDurableEncoding {
  ...
  /**
   * A predicate that allows us to call out values we want to be stored individually.  This is useful for any value
   * which
  ...
  /**
   * An extension of {@link Iterator}, which provides for skipping elements without ever providing a value.  This can
   * allow
